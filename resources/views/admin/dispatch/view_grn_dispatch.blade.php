...
use \MongoDB\BSON\ObjectID as MongoId;
        ...
        $Received_by= \DB::table('login_users')->where(['_id'=>new MongoId($data['swh'])])->first();
       
        $Supplier = \DB::table('login_users')->where(['_id'=>new MongoId($data['wh'])])->first();
?>
...
<table width="700px" border="0" cellspacing="0" cellpadding="5" style="font-family:Arial, Helvetica, sans-serif; font-size:12px; margin:0 auto; border:1px solid #000">
  ...
  <tr>
    <td style="border-bottom:1px solid #000"><table width="100%" border="0" cellspacing="0" cellpadding="5">
      <tr>
        <td><strong>GRN No.:</strong>{{$data['utility']}}-{{date('Y',$data['dispatch_created_time'])}}-00{{$data['grn_no']}}</td>
        <td><strong>Invoice No.:</strong>{{$data['mrn_ref']}}</td>
        <td><strong>GRN Date:</strong>{{date('d-m-Y',$data['dispatch_created_time'])}}</td>
      </tr>
      <tr>
      ...
      <tr>
        <td><strong>Order No.:</strong>{{$data['dl_ch_ref']}}</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
      </tr>
    </table></td>
