...
@section('content')
            ...
            <div class="box-body">
              <table class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th>Mobile No.</th>
                  <th>Location</th>
                  <th>MRN Process Date</th>
                  <th>MRN Approved Date(Authority 1)</th>
                  <th>Status</th>
                  <th>Action</th>
                </tr>
                </thead>
                <tbody>
                  ...
                  @foreach($data as $value)
                    <tr>
                      ...
                      <td>
                        {{date('Y-m-d & h:s:i',strtotime($value['mrn_authorized_1_update_time']))}}
                      </td>
                      <td>
                        @if($value['mrn_authorized_2_status']=='Pending')
                          <a href="#" onclick="function_model('{{$value['_id']}}')"><span class="badge bg-yellow">{{$value['mrn_authorized_2_status']}}</span></a>
                        @else
                          <a href="#" ><span class="badge bg-green">{{$value['mrn_authorized_2_status']}}</span></a>
                        @endif
  ...
  <div class="modal-dialog">
    ...
    <form action="{{ url('/')}}/status_update_mrn_a_2" method="post" data-parsley-validate="parsley">
      {!! csrf_field() !!}
